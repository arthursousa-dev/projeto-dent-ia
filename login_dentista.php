...
if (isset($_SESSION['perfil']) && $_SESSION['perfil'] === 'dentista') {
    header('Location: dentista_dashboard.php'); exit;
}
$erro = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $usuario = autenticarUsuario(trim($_POST['email']??''), $_POST['senha']??'', 'dentista');
    if ($usuario) {
        $_SESSION['usuario'] = $usuario['nome'];
        $_SESSION['perfil']  = 'dentista';
...
<head>
  ...
  <title>Área do Dentista — DENT IA</title>
  <script src="js/accessibility.js"></script>
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.13.1/font/bootstrap-icons.min.css">
  <link rel="stylesheet" href="css/style.css">
  <style>
    :root{--cor-role:#00c9a7;--cor-role-fundo:rgba(0,201,167,0.10);}
    .login-wrap{min-height:100vh;display:flex;}
    .login-esq{width:400px;flex-shrink:0;background:linear-gradient(160deg,#0b2845 0%,#0d5a4a 100%);display:flex;flex-direction:column;align-items:flex-start;justify-content:center;padding:60px 48px;position:relative;overflow:hidden;}
    .login-esq::before{content:'';position:absolute;bottom:-80px;right:-80px;width:300px;height:300px;border-radius:50%;background:radial-gradient(circle,rgba(0,201,167,0.25) 0%,transparent 70%);}
    .logo{font-family:var(--fonte-titulo);font-size:36px;font-weight:900;color:#fff;margin-bottom:8px;}.logo span{color:#4fffde;}
    .logo-sub{color:rgba(255,255,255,0.65);font-size:14px;margin-bottom:48px;}
    .role-badge{display:inline-flex;align-items:center;gap:10px;background:rgba(0,201,167,0.20);border:1px solid rgba(79,255,222,0.4);border-radius:var(--raio-pilula);padding:10px 18px;color:#4fffde;font-weight:600;font-size:14px;margin-bottom:24px;}
    ...
    .grupo input:focus{outline:none;border-color:var(--cor-role);box-shadow:0 0 0 3px rgba(0,201,167,0.20);}
    ...
    .btn-entrar{width:100%;padding:13px;background:var(--cor-role);color:#0b2845;border:none;border-radius:var(--raio-pilula);font-family:var(--fonte);font-size:15px;font-weight:700;cursor:pointer;transition:background var(--transicao),transform var(--transicao);margin-bottom:16px;}
    .btn-entrar:hover{background:#00a98a;transform:translateY(-1px);}
...
<main class="login-wrap" id="c">
  <div class="login-esq" role="complementary">
    ...
    <div class="role-badge"><i class="bi bi-emoji-smile-fill" aria-hidden="true"></i> Área do Dentista</div>
    <ul class="login-desc">
      <li><i class="bi bi-check2" aria-hidden="true"></i> Visualize sua agenda do dia</li>
      <li><i class="bi bi-check2" aria-hidden="true"></i> Acesse prontuários com odontograma 3D</li>
      <li><i class="bi bi-check2" aria-hidden="true"></i> Registre tratamentos e evoluções</li>
      <li><i class="bi bi-check2" aria-hidden="true"></i> Acompanhe histórico clínico completo</li>
    </ul>
  </div>
  <div class="login-dir">
    <div class="caixa">
      <h1>Bem-vindo, Doutor(a) <i class="bi bi-emoji-smile-fill" aria-hidden="true"></i></h1>
      <p>Acesse sua área clínica para continuar</p>
      ...
      <form method="POST" novalidate>
        <?= csrfCampo() ?>
        <div class="grupo"><label for="email">E-mail profissional</label><input type="email" id="email" name="email" placeholder="dentista@dentai.com" required autocomplete="email" value="<?=limpar($_POST['email']??'')?>"></div>
        ...
        <button type="submit" class="btn-entrar">Entrar como Dentista →</button>
      </form>
      ...
      <div class="credenciais" role="note">
        ...
        <div class="cred-linha"><span class="cred-chave">E-mail</span><span class="cred-val">dentista@dentai.com</span></div>
