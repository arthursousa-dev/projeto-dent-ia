...
if (isset($_SESSION['perfil']) && $_SESSION['perfil'] === 'dono') {
    header('Location: dono_dashboard.php'); exit;
}
$erro = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $usuario = autenticarUsuario(trim($_POST['email']??''), $_POST['senha']??'', 'dono');
    if ($usuario) {
        $_SESSION['usuario'] = $usuario['nome'];
        $_SESSION['perfil']  = 'dono';
...
<head>
  ...
  <title>Área do Gestor — DENT IA</title>
  <script src="js/accessibility.js"></script>
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.13.1/font/bootstrap-icons.min.css">
  <link rel="stylesheet" href="css/style.css">
  <style>
    :root{--cor-role:#0b2845;--cor-role-fundo:rgba(11,40,69,0.10);}
    .login-wrap{min-height:100vh;display:flex;}
    .login-esq{width:400px;flex-shrink:0;background:linear-gradient(160deg,#050f1c 0%,#0b2845 60%,#194e82 100%);display:flex;flex-direction:column;align-items:flex-start;justify-content:center;padding:60px 48px;position:relative;overflow:hidden;}
    .login-esq::before{content:'';position:absolute;top:-60px;right:-60px;width:280px;height:280px;border-radius:50%;background:radial-gradient(circle,rgba(245,166,35,0.20) 0%,transparent 70%);}
    .logo{font-family:var(--fonte-titulo);font-size:36px;font-weight:900;color:#fff;margin-bottom:8px;}.logo span{color:#f5a623;}
    .logo-sub{color:rgba(255,255,255,0.65);font-size:14px;margin-bottom:48px;}
    .role-badge{display:inline-flex;align-items:center;gap:10px;background:rgba(245,166,35,0.15);border:1px solid rgba(245,166,35,0.4);border-radius:var(--raio-pilula);padding:10px 18px;color:#fcd34d;font-weight:600;font-size:14px;margin-bottom:24px;}
    ...
    .grupo input:focus{outline:none;border-color:#f5a623;box-shadow:0 0 0 3px rgba(245,166,35,0.18);}
    ...
    .btn-entrar{width:100%;padding:13px;background:var(--primario);color:#fff;border:none;border-radius:var(--raio-pilula);font-family:var(--fonte);font-size:15px;font-weight:700;cursor:pointer;transition:background var(--transicao),transform var(--transicao);margin-bottom:16px;}
    .btn-entrar:hover{background:#194e82;transform:translateY(-1px);}
    .link-volta{display:block;text-align:center;color:var(--texto-secundario);font-size:13px;text-decoration:none;}.link-volta:hover{color:var(--primario);}
    ...
    .cred-chave{font-weight:600;color:var(--texto-principal);}.cred-val{font-family:monospace;color:#0b2845;font-weight:700;}
...
<main class="login-wrap" id="c">
  <div class="login-esq" role="complementary">
    ...
    <div class="role-badge"><i class="bi bi-award-fill" aria-hidden="true"></i> Área do Gestor</div>
    <ul class="login-desc">
      <li><i class="bi bi-check2" aria-hidden="true"></i> Dashboard financeiro completo</li>
      <li><i class="bi bi-check2" aria-hidden="true"></i> Relatórios e indicadores da clínica</li>
      <li><i class="bi bi-check2" aria-hidden="true"></i> Gestão de dentistas e equipe</li>
      <li><i class="bi bi-check2" aria-hidden="true"></i> Configurações gerais do sistema</li>
    </ul>
  </div>
  <div class="login-dir">
    <div class="caixa">
      <h1>Painel Administrativo <i class="bi bi-award-fill" aria-hidden="true"></i></h1>
      <p>Acesso restrito ao gestor e proprietário</p>
      ...
      <form method="POST" novalidate>
        <?= csrfCampo() ?>
        <div class="grupo"><label for="email">E-mail do gestor</label><input type="email" id="email" name="email" placeholder="dono@dentai.com" required autocomplete="email" value="<?=limpar($_POST['email']??'')?>"></div>
        ...
        <div style="text-align:right;margin:-8px 0 18px"><a href="esqueci_senha.php" style="font-size:13px;color:#f5a623;font-weight:600;text-decoration:none;">Esqueci minha senha</a></div>
        <button type="submit" class="btn-entrar">Entrar como Gestor →</button>
      </form>
      ...
      <div class="credenciais" role="note">
        ...
        <div class="cred-linha"><span class="cred-chave">E-mail</span><span class="cred-val">dono@dentai.com</span></div>
